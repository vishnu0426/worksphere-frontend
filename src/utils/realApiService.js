...
const realApiService = {
  ...
  organizations: {
    ...
    // Add method to get pending invitations using the correct backend endpoint
  ...
  projects: {
    ...
    getStats: async (projectId, organizationId) => {
      const qs = buildQS({ organization_id: organizationId });
      const resp = await fetch(`${API_BASE_URL}/api/v1/projects/${projectId}/stats${qs}`, {
        headers: getAuthHeaders(),
      });
      const r = await handle(resp);
      return r;
  ...
  columns: {
    getByBoard: async (boardId) => {
      ...
      console.log(`🔧 API URL: ${API_BASE_URL}/api/v1/boards/${boardId}/columns`);
      ...
      if (!resp.ok) {
        ...
        console.error(`❌ Failed to fetch columns: ${resp.status} - ${errorText}`);
        throw new Error(`Failed to fetch columns: ${resp.status} - ${errorText}`);
      }

      const r = await handle(resp);
      console.log(`🔧 Columns response:`, r);

      // Ensure we return an array
      const columns = r.data || r || [];
  ...
  cards: {
    ...
    getById: async (id) => {
      const resp = await fetchWithCredentials(`${API_BASE_URL}/api/v1/cards/${id}`);
      const result = await handle(resp);
    ...
    create: async (columnId, data) => {
      ...
      console.log('🔧 Creating card via realApiService:', payload);
      const resp = await fetchWithCredentials(`${API_BASE_URL}/api/v1/cards/column`, {
        method: 'POST',
        body: JSON.stringify(payload),
      });
      const result = await handle(resp);
      console.log('🔧 Card creation API response:', result);
      ...
      if (result && typeof result === 'object') {
        // If backend returns { success: true, data: {...} }, extract the data
        if (result.data) {
          console.log('✅ Card created successfully via realApiService:', result.data);
          return { data: result.data };
        }
        // If backend returns the card data directly, wrap it
        console.log('✅ Card created successfully (direct format):', result);
        return { data: result };
      }
    ...
    move: async (cardId, targetColumnId, position = null) => {
      ...
      const resp = await fetchWithCredentials(`${API_BASE_URL}/api/v1/cards/${cardId}/move`, {
  ...
  notifications: {
    ...
    getStatistics: async (organizationId = null) => {
      const params = organizationId ? `?organization_id=${organizationId}` : '';
      const resp = await fetch(`${API_BASE_URL}/api/v1/notifications/in-app/statistics${params}`, {
        headers: getAuthHeaders(),
      });
      const result = await handle(resp);
  ...
  meetings: {
    getAll: async (params = {}) => {
      const qs = new URLSearchParams(params).toString();
      const url = qs ? `${API_BASE_URL}/api/v1/meetings?${qs}` : `${API_BASE_URL}/api/v1/meetings`;
      const resp = await fetchWithCredentials(url);
      return handle(resp);
    },
    getById: async (meetingId) => {
      const resp = await fetch(`${API_BASE_URL}/api/v1/meetings/${meetingId}`, {
        headers: getAuthHeaders(),
      });
      return handle(resp);
    },
    getByProject: async (projectId) => {
      const resp = await fetch(`${API_BASE_URL}/api/v1/meetings/project/${projectId}`, {
        headers: getAuthHeaders(),
      });
      return handle(resp);
    },
    create: async (meetingData) => {
      const resp = await fetch(`${API_BASE_URL}/api/v1/meetings/create`, {
        method: 'POST',
        headers: getAuthHeaders(),
    ...
    createInstant: async (meetingData) => {
      const resp = await fetch(`${API_BASE_URL}/api/v1/meetings/instant`, {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify(meetingData),
      });
    ...
    update: async (meetingId, updateData) => {
      const resp = await fetch(`${API_BASE_URL}/api/v1/meetings/${meetingId}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
        body: JSON.stringify(updateData),
    ...
    integrateCalendar: async (integrationData) => {
      const resp = await fetch(`${API_BASE_URL}/api/v1/meetings/calendar/integrate`, {
        method: 'POST',
        headers: getAuthHeaders(),
  ...
  organizationSettings: {
    get: async (organizationId) => {
      const resp = await fetch(`${API_BASE_URL}/api/v1/organizations/${organizationId}/settings`, {
        headers: getAuthHeaders(),
      });
      return handle(resp);
    },
    update: async (organizationId, settings) => {
      const resp = await fetch(`${API_BASE_URL}/api/v1/organizations/${organizationId}/settings`, {
        method: 'PUT',
        headers: getAuthHeaders(),
    ...
    updateProjectCreationPermissions: async (organizationId, permissions) => {
      const resp = await fetch(`${API_BASE_URL}/api/v1/organizations/${organizationId}/settings/project-permissions`, {
        method: 'PUT',
        headers: getAuthHeaders(),
  ...
  integrations: {
    getAll: async (organizationId) => {
      const resp = await fetch(`${API_BASE_URL}/api/v1/integrations?organization_id=${organizationId}`, {
        headers: getAuthHeaders(),
      });
      return handle(resp);
    },
    getById: async (integrationId) => {
      const resp = await fetch(`${API_BASE_URL}/api/v1/integrations/${integrationId}`, {
        headers: getAuthHeaders(),
      });
      return handle(resp);
    },
    create: async (organizationId, integrationData) => {
      const resp = await fetch(`${API_BASE_URL}/api/v1/integrations`, {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({ organization_id: organizationId, ...integrationData }),
      });
      return handle(resp);
    },
    update: async (integrationId, updateData) => {
      const resp = await fetch(`${API_BASE_URL}/api/v1/integrations/${integrationId}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
        body: JSON.stringify(updateData),
    ...
    toggle: async (integrationId, enabled) => {
      const resp = await fetch(`${API_BASE_URL}/api/v1/integrations/${integrationId}/toggle`, {
        method: 'POST',
        headers: getAuthHeaders(),
    ...
    sync: async (integrationId) => {
      const resp = await fetch(`${API_BASE_URL}/api/v1/integrations/${integrationId}/sync`, {
        method: 'POST',
        headers: getAuthHeaders(),
      });
      return handle(resp);
    ...
    getTemplates: async () => {
      const resp = await fetch(`${API_BASE_URL}/api/v1/integrations/templates`, {
        headers: getAuthHeaders(),
      });
      return handle(resp);
    },
  ...
  kanban: {
    getProjectBoard: async (projectId) => {
      const resp = await fetchWithCredentials(`${API_BASE_URL}/api/v1/kanban/projects/${projectId}/board`);
      return handle(resp);
    },

    createColumn: async (boardId, columnData) => {
      const resp = await fetchWithCredentials(`${API_BASE_URL}/api/v1/kanban/boards/${boardId}/columns`, {
        method: 'POST',
    ...
    updateColumn: async (columnId, columnData) => {
      const resp = await fetchWithCredentials(`${API_BASE_URL}/api/v1/kanban/columns/${columnId}`, {
        method: 'PUT',
        body: JSON.stringify(columnData),
      });
      return handle(resp);
    ...
    createCard: async (columnId, cardData) => {
      const resp = await fetchWithCredentials(`${API_BASE_URL}/api/v1/kanban/columns/${columnId}/cards`, {
        method: 'POST',
    ...
    updateCard: async (cardId, cardData) => {
      const resp = await fetchWithCredentials(`${API_BASE_URL}/api/v1/kanban/cards/${cardId}`, {
        method: 'PUT',
        body: JSON.stringify(cardData),
      });
      return handle(resp);
    ...
    moveCard: async (cardId, targetColumnId, position = null) => {
      const resp = await fetchWithCredentials(`${API_BASE_URL}/api/v1/kanban/cards/${cardId}/move`, {
        method: 'PUT',
        body: JSON.stringify({
