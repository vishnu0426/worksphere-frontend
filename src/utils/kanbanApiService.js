...
const kanbanApiService = {
  ...
  board: {
    getOrCreateForProject: async (projectId) => {
      ...
      console.log(`🔧 API URL: ${API_BASE_URL}/api/v1/kanban/projects/${projectId}/board`);
      ...
      if (!resp.ok) {
        ...
        console.error(`❌ Failed to get/create board: ${resp.status} - ${errorText}`);
        throw new Error(`Failed to get/create board: ${resp.status} - ${errorText}`);
      }
      ...
      // Ensure we return the board data in a consistent format
      if (result && (result.id || result.data?.id)) {
        return result;
      } else {
        console.error('❌ Invalid board response structure:', result);
        throw new Error('Invalid board response structure');
      }
    }
  },
