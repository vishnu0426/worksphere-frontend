...
export const getNotificationStatistics = async () => {
  try {
    ...
    const result = await realApiService.notifications.getStatistics();
    ...
    if (result && result.data) {
      ...
      // Calculate statistics from current notifications
      const notifications = await getNotifications();
      if (notifications.success && notifications.data) {
        const stats = calculateNotificationStatistics(notifications.data);
        return {
          success: true,
          data: stats
        };
...
// Calculate statistics from notification array
const calculateNotificationStatistics = (notifications) => {
  const total = notifications.length;
  const unread = notifications.filter(n => !n.read && !n.read_at).length;
  const readPercentage = total > 0 ? ((total - unread) / total * 100).toFixed(1) : 0;
  ...
  const categoryBreakdown = {
    ...
    project_milestone: notifications.filter(n => n.type === 'project_milestone').length,
    team_member_added: notifications.filter(n => n.type === 'team_member_added').length,
  ...
  return {
    ...
    read_percentage: parseFloat(readPercentage),
...
// Enhanced team member assignment notification for project creation
export const notifyProjectTeamAssignment = async (projectData, teamMemberIds, projectManagerId, creatorId, organizationId) => {
  try {
    const notifications = [];
    ...
    if (teamMemberIds && teamMemberIds.length > 0) {
      for (const memberId of teamMemberIds) {
        if (memberId !== creatorId) { // Don't notify the creator
          const notification = await realApiService.notifications.sendTeamMemberAddedNotification({
            user_id: memberId,
    ...
    // Notify project manager separately if different from team members
    if (projectManagerId && projectManagerId !== creatorId && !teamMemberIds?.includes(projectManagerId)) {
      const notification = await realApiService.notifications.sendTeamMemberAddedNotification({
        user_id: projectManagerId,
        project_name: projectData.name,
        project_id: projectData.id,
        new_member_name: 'You (as Project Manager)',
        added_by: creatorId,
        organization_id: organizationId
      });
      notifications.push(notification);
    ...
    console.error('Failed to send project team assignment notifications:', error);
    return { success: false, error: error.message };
  }
};
