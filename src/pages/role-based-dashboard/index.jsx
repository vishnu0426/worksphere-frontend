...
const RoleBasedDashboard = () => {
  ...
  const [canCreateProjects, setCanCreateProjects] = useState(true); // Default to true for backward compatibility
    ...
    const loadDashboardData = async () => {
      try {
        ...
        if (userResult.data.user) {
          ...
          if (userResult.data.user.role === 'admin' && organizationId) {
            try {
              const orgSettings = await apiService.organizations.getSettings(organizationId);
              const isOwner = userResult.data.user.role === 'owner';
            ...
            setCanCreateProjects(userResult.data.user.role === 'owner');
          }
        } else {
        ...
        if (organizationId) {
          ...
          try {
            ...
            if (Array.isArray(activitiesResult)) {
              ...
              console.log('✅ Activities loaded from API:', activitiesResult.length);
            } else {
              // No activities available - show empty state
              setActivities([]);
              console.log('ℹ️ No activities available from API');
            }
  ...
  const handleCreateProject = async (projectData) => {
    try {
      ...
      let toastMessage = `Project "${projectData.name}" created successfully!`;
          ...
          <div className='flex items-center gap-4'>
            <div className='flex items-center gap-3'>
              ...
              <span className='text-sm font-medium text-slate-700'>
                {currentUser
                  ? `Welcome, ${currentUser.firstName || currentUser.displayName?.split(' ')[0] || currentUser.name?.split(' ')[0] || currentUser.email?.split('@')[0] || 'User'}!`
                  : 'Welcome!'}
              </span>
            </div>
      ...
      <div className='max-w-7xl mx-auto px-6 py-8'>
        ...
        <div className='grid grid-cols-1 xl:grid-cols-3 gap-8 mb-10'>
          ...
          <div className='xl:col-span-2 space-y-8'>
            ...
            <QuickActions
              userRole={userRole}
              onCreateProject={canCreateProjects ? handleOpenCreateProject : undefined}
