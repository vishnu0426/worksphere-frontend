...
const TeamMembers = () => {
  ...
  const loadTeamMembers = useCallback(async () => {
    ...
    try {
      ...
      try {
        // First try the team service
        activeMembers = await teamService.getTeamMembers(
        ...
        console.log('Loaded active members via teamService:', activeMembers);
      } catch (error) {
        console.warn('teamService.getTeamMembers failed, trying direct API:', error);

        // Fallback to direct API call
        try {
          const realApiService = (await import('../../utils/realApiService.js')).default;
          const membersResponse = await realApiService.organizations.getMembers(currentOrganization.id);
          activeMembers = membersResponse || [];
          console.log('Loaded active members via direct API:', activeMembers);
        } catch (directError) {
      ...
      try {
        ...
        pendingMembers = await teamService.getPendingInvitations(
          currentOrganization.id
        );
      ...
      // Ensure we have at least the current user as an active member
      if (activeMembers.length === 0 && currentUser) {
        console.log('No active members found, adding current user as fallback');
        activeMembers = [{
      ...
      // Get emails of active members to filter out duplicate invitations
      const activeMemberEmails = new Set(activeMembers.map(member => member.email));

      // Filter out pending invitations for users who are already active members
      const filteredPendingMembers = pendingMembers.filter(invite =>
        !activeMemberEmails.has(invite.email)
      );
      ...
      const allMembers = [
        ...
        ...filteredPendingMembers.map(invite => ({
          id: invite.id,
          email: invite.email,
  ...
  }, [currentOrganization?.id, currentUser, userRole, searchQuery, roleFilter]);
  ...
  useEffect(() => {
    loadTeamMembers();
  }, [currentOrganization?.id, searchQuery, roleFilter, loadTeamMembers]);
