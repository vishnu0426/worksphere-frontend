...
const MemberDashboard = () => {
  ...
  const {
    userProfile: _userProfile, // eslint-disable-line no-unused-vars
    currentOrganization: hookCurrentOrganization,
    availableOrganizations: _availableOrganizations, // eslint-disable-line no-unused-vars
    loading: _profileLoading, // eslint-disable-line no-unused-vars
  } = useUserProfile();
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      try {
        ...
        console.log('🏢 Member Dashboard - Organization ID:', organizationId);
        console.log('📋 Member Dashboard - Current Session:', sessionService.currentSession);
        console.log('🔍 Member Dashboard - Current Organization:', sessionService.getCurrentOrganization());
        ...
        if (organizationId) {
          ...
          try {
            ...
            console.log('🔍 Organization context:', sessionService.getCurrentOrganization());

            // Get active members
            const activeMembers = await teamService.getTeamMembers(organizationId);
            console.log('✅ Active members loaded:', activeMembers);
            console.log('✅ Active members count:', activeMembers?.length || 0);

            // Check if we're looking at the right organization (Medi HealthCare)
            if (organizationId === 'c4240d6f-e792-42ac-816e-caf29162246c') {
              console.log('🎯 This is Medi HealthCare organization - should show Charumathi and Ragul');
            } else {
              console.log('⚠️ This is NOT Medi HealthCare organization. Current org:', organizationId);
            }
            ...
            console.log('📋 Pending invitations count:', pendingInvitations?.length || 0);
          ...
          console.warn('No organization ID available - user may not be properly associated with an organization');
          setProjects([]);
          setTeamMembers([]);
        }
        ...
        try {
          ...
          const activitiesResult = await apiService.activities.getAll(organizationId);
        ...
        try {
          ...
          const tasksResult = await apiService.tasks.getAssignedToUser(currentUser.id, organizationId);
  ...
  useEffect(() => {
    if (projects.length > 0 || teamMembers.length > 0 || tasks.length > 0) {
      ...
      if (!dashboardData || projects.length > 0 || tasks.length > 0) {
        setDashboardData(calculatedStats);
      }
    }
  }, [projects.length, teamMembers.length, activities.length, tasks.length, dashboardData]);
  ...
  const getMemberKPIData = useCallback(() => {
    ...
    const taskCount = tasks.length || realData.pending_tasks || 0; // eslint-disable-line no-unused-vars
    ...
    return [
      {
        ...
        changeType: projectCount > 0 ? 'positive' : 'neutral',
        icon: 'FolderOpen',
        color: 'primary',
      },
      {
      ...
      {
        title: 'My Activity',
        value: (activities.length || realData.total_activity || 0).toString(),
        change: activities.length > 0 ? `+${activities.length}` : '+0',
        changeType: activities.length > 0 ? 'positive' : 'neutral',
        icon: 'Activity',
        color: 'warning',
      },
    ];
  }, [dashboardData, projects.length, teamMembers.length, tasks.length, activities.length]);
    ...
    <div className='min-h-screen bg-gradient-to-br from-slate-50 to-blue-50'>
      ...
      <div className='bg-white shadow-sm border-b border-slate-200'>
        <div className='max-w-7xl mx-auto px-6 py-4'>
          <div className='flex items-center justify-between text-sm text-slate-600'>
            <span>My Projects: {projects.length || dashboardData?.total_projects || 0}</span>
            <span>•</span>
            <span>Team Members: {teamMembers.length || dashboardData?.total_members || 0}</span>
            <span>•</span>
            <span>My Activity: {activities.length || dashboardData?.total_activity || 0}</span>
          </div>
        </div>
      </div>
      ...
      <main id='main-content' className='max-w-7xl mx-auto px-6 py-8'>
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-8'>
          <TaskSummary tasks={tasks} userRole='member' loading={tasksLoading} />
