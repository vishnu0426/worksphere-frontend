...
const BoardColumn = ({
  ...
  // Handle both 'title' and 'name' properties from backend
  const [columnTitle, setColumnTitle] = useState(column.title || column.name || 'Untitled');
  ...
  const handleTitleSave = () => {
    const currentTitle = column.title || column.name || 'Untitled';
    if (columnTitle.trim() && columnTitle !== currentTitle) {
      onEditColumn(column.id, { title: columnTitle.trim(), name: columnTitle.trim() });
    }
    setIsEditing(false);
  };
  ...
  const handleKeyPress = (e) => {
    if (e.key === 'Enter') {
      ...
      setColumnTitle(column.title || column.name || 'Untitled');
      setIsEditing(false);
    }
  };
