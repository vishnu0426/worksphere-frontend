...
const ProjectOverview = () => {
  ...
  useEffect(() => {
    const loadProjectData = async () => {
      try {
        ...
        // Load project data, stats, and team members in parallel
        const [projectData, projectStatsData, teamMembersData] = await Promise.all([
          apiService.projects.getById(projectId),
          realApiService.projects.getStats(projectId, currentOrg.id).catch(err => {
            console.warn('Failed to load project stats:', err);
            return null;
          }),
          realApiService.projects.getTeamMembers(projectId, currentOrg.id).catch(err => {
        ...
        console.log('Project stats loaded:', projectStatsData);
        console.log('Project team members loaded:', teamMembersData);
        ...
        const transformedProject = {
          ...
          team: {
            totalMembers: projectStatsData?.member_count || 0,
            activeMembers: teamMembersData?.length || 0,
            tasksCompleted: projectStatsData?.completed_tasks || 0,
          },
        };
  ...
  // Real team members data - loaded from project stats API
