...
const ProjectOverview = ({ project, userRole, onSwitchTab }) => {
  ...
  // Load real project stats and team members
  const loadRealProjectData = useCallback(async () => {
    if (!project?.id) return;

    try {
      // Get current organization
      const sessionService = (await import('../../../utils/sessionService')).default;
      const currentOrg = sessionService.getCurrentOrganization();

      if (!currentOrg?.id) {
        console.warn('No organization found for loading project stats');
        return;
      }
      ...
      // Load project stats and team members in parallel
      const [statsData, membersData] = await Promise.all([
        realApiService.projects.getStats(project.id, currentOrg.id).catch(err => {
          console.warn('Failed to load project stats:', err);
          return null;
        }),
        realApiService.projects.getTeamMembers(project.id, currentOrg.id).catch(err => {
          console.warn('Failed to load project team members:', err);
          return [];
        })
      ]);

      console.log('Project stats loaded:', statsData);
      console.log('Project team members loaded:', membersData);

      setProjectStats(statsData);
      setTeamMembers(membersData || []);
    } catch (error) {
      console.error('Error loading real project data:', error);
    }
  }, [project?.id]);
  ...
  useEffect(() => {
    if (project) {
      loadProjectData();
      loadRealProjectData(); // Load real stats and team members
    }
  }, [project, loadProjectData, loadRealProjectData]);
  ...
  // Calculate dynamic progress from real project stats
  const calculateProjectProgress = useCallback(() => {
    // Use real project stats if available
    if (projectStats?.total_tasks > 0) {
      return Math.round((projectStats.completed_tasks / projectStats.total_tasks) * 100);
    }

    // Fallback to project tasks if available
        ...
        <div className='bg-card rounded-lg border border-border p-6'>
          ...
          <div className='space-y-3'>
            <div className='flex items-center justify-between'>
              ...
              <span className='font-medium text-foreground'>
                {projectStats?.member_count || teamMembers.length}
              </span>
            </div>
            <div className='flex items-center justify-between'>
