...
const RoleBasedHeader = ({
  ...
  useEffect(() => {
    console.log('🔍 HEADER DEBUG: User profile data:', {
      propCurrentUser,
      userProfile,
      currentUser,
  ...
  }, [propCurrentUser, userProfile, currentUser]);

  // Always prefer hook data over prop data for organization
  const organization = currentOrganization || propCurrentOrganization;
  ...
  useEffect(() => {
    const checkAdminPermissions = async () => {
      if (disableAsync || !actualUserRole || !organization?.id) return;

      if (actualUserRole === 'admin') {
        try {
          const orgSettings = await apiService.organizations.getSettings(organization.id);
          const adminCanCreate = orgSettings?.allow_admin_create_projects === true;
          setCanCreateProjects(adminCanCreate);
        } catch (error) {
        ...
        setCanCreateProjects(actualUserRole === 'owner');
      }
    };

    checkAdminPermissions();
  }, [actualUserRole, organization?.id, disableAsync]);
  ...
  return (
    <header className='fixed top-0 left-0 right-0 z-50 bg-white border-b border-slate-200 shadow-sm backdrop-blur-sm bg-white/95'>
                    ...
                    {isProjectDropdownOpen && (
                      <div className='absolute top-full left-0 mt-1 w-80 bg-popover border border-border rounded-md shadow-elevated z-1010'>
                        <div className='p-2'>
                          ...
                          {/* Create New Project - Only for users with permission */}
                          {canCreateProjects && (
                            <>
                              <button
          ...
          <div className='relative' ref={userDropdownRef}>
            ...
            >
              ...
              <div className='hidden md:block text-left'>
                ...
                <div className='text-xs text-text-secondary'>
                  {actualUserRole ? actualUserRole.charAt(0).toUpperCase() + actualUserRole.slice(1) : 'Member'}
                </div>
              </div>
              <Icon
                name='ChevronDown'
