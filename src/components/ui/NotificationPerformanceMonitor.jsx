...
const NotificationPerformanceMonitor = ({ isOpen, onClose }) => {
  ...
  const loadStatistics = async () => {
    setLoading(true);
    try {
      // Get real notification statistics from API
      ...
      if (result.success && result.data) {
        ...
        console.log('✅ Loaded real notification statistics:', result.data);
      } else {
        console.warn('⚠️ Failed to load notification statistics, using fallback');
      ...
      setStatistics({
        ...
        unread_notifications: 0,
        read_percentage: 0,
        priority_breakdown: { urgent: 0, high: 0, normal: 0, low: 0 },
        category_breakdown: { welcome: 0, task_assigned: 0, task_reminder: 0, task_updated: 0, project_update: 0, project_milestone: 0, team_member_added: 0, system: 0 }
      });
      setLastUpdated(new Date());
    }
    setLoading(false);
