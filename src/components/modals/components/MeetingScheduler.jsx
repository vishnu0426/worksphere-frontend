...
const MeetingScheduler = ({ projectData, tasks, organizationMembers }) => {
  ...
  const loadProjectMeetings = async () => {
    try {
      if (!projectData?.id) {
        console.warn('No project ID available for loading meetings');
        return;
      }

      console.log('Loading meetings for project:', projectData.id);

      // Try to load real meetings from API
      try {
        const meetingsResult = await realApiService.meetings.getByProject(projectData.id);

        if (meetingsResult && Array.isArray(meetingsResult)) {
          const formattedMeetings = meetingsResult.map(meeting => ({
            id: meeting.id,
            type: meeting.meeting_type || 'general',
            title: meeting.title,
            date: meeting.scheduled_at ? meeting.scheduled_at.split('T')[0] : '',
            time: meeting.scheduled_at ? meeting.scheduled_at.split('T')[1]?.substring(0, 5) : '',
          ...
          console.log('✅ Real meetings loaded:', formattedMeetings);
        } else {
          throw new Error('Invalid meetings response');
        }
      } catch (apiError) {
        console.warn('API meetings not available, using fallback:', apiError.message);

        // Fallback meetings if API is not available
        const fallbackMeetings = [
          {
            id: 'meeting-fallback-1',
            type: 'kickoff',
            title: 'Project Kickoff Meeting',
            date: new Date().toISOString().split('T')[0],
            time: '10:00',
            duration: 90,
            attendees: organizationMembers?.slice(0, 3).map(m => m.id) || [],
            status: 'scheduled',
            meetingUrl: await generateMeetingUrl({ type: 'kickoff' })
          }
        ];
        setActiveMeetings(fallbackMeetings);
        console.log('Fallback meetings loaded:', fallbackMeetings);
      }
    } catch (error) {
  ...
  const scheduleMeeting = async (meetingData) => {
    try {
      ...
      const scheduledDateTime = meetingData.date && meetingData.time
        ? `${meetingData.date}T${meetingData.time}:00Z`
        : new Date().toISOString();

      const apiMeetingData = {
        project_id: projectData.id,
        title: meetingData.title || `${meetingData.type} Meeting`,
        description: meetingData.description || '',
        meeting_type: meetingData.type || 'general',
      ...
      try {
        // Try to create meeting via API
        const createdMeeting = await realApiService.meetings.create(apiMeetingData);

        if (createdMeeting) {
          console.log('✅ Meeting created via API:', createdMeeting);
          return {
            id: createdMeeting.id,
            type: createdMeeting.meeting_type,
            title: createdMeeting.title,
            date: createdMeeting.scheduled_at?.split('T')[0] || '',
            time: createdMeeting.scheduled_at?.split('T')[1]?.substring(0, 5) || '',
            duration: createdMeeting.duration_minutes,
            attendees: createdMeeting.participants || [],
            status: createdMeeting.status || 'scheduled',
            meetingUrl: createdMeeting.meeting_link || await generateMeetingUrl(meetingData),
            createdAt: createdMeeting.created_at || new Date().toISOString()
          };
        } else {
          throw new Error('Invalid meeting creation response');
        }
      } catch (apiError) {
        console.warn('API meeting creation failed, using fallback:', apiError.message);

        // Fallback meeting creation
        const meeting = {
          id: `meeting-${Date.now()}`,
          ...meetingData,
          status: 'scheduled',
        ...
        console.log('Fallback meeting created:', meeting);
        return meeting;
      }
    } catch (error) {
      console.error('Failed to schedule meeting:', error);
